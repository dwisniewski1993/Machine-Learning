...
class LSTMModel:
    ...
    def train(self, retrain: bool = False) -> None:
        ...
        if os.path.exists(self.data_name + '__LSTM_model'):
            self.logger.info('Trained LSTM model detected')
            if retrain:
                self.logger.info('Start training LSTM Network.....')
    ...
    def score(self, data: np.array) -> np.array:
        ...
        """
        self.logger.info('Calculate LSTM Score...')
    ...
    def calculate_threshold(self, helth: np.array):
        ...
        :return: None
        """
        self.logger.info('Calculate LSTM threshold...')
    ...
    def anomaly_score(self, pred: np.array):
        ...
        if len(pred) == len(actual):
            ...
            with open(r'LSTM_detected_' + self.data_name + '.csv', 'w') as file:
                for each in dists:
