...
class RRRegressor:
    ...
    def __init__(self, trainfile):
        """
        Kernel Ridge Regression Constructor
        Loading and preparing data
        :param trainfile: Poznań flats data tsv path
        """
        log.getLogger().setLevel(log.INFO)
        log.info('RR Regressor')
    ...
    def output(self):
        ...
        :return: None
        """
        y_pred = self.rrr.predict(self.X_test)
    ...
    def grid_search(self):
        ...
        classifier = GridSearchCV(KernelRidge(), hyperparam_grid, cv=5, iid=False, scoring='explained_variance')
    ...
    def train_model(self):
        ...
        :return: None
        """
        self.rrr = KernelRidge(kernel=self.grided_params[0], gamma=self.grided_params[1]).fit(self.X_train,
                                                                                              self.Y_train)
